  ...
  <Dropdown ref="dropdown" @on-click="handleClick" :class="hideTitle ? '' : 'collased-menu-dropdown'" :transfer="hideTitle" :placement="placement">
    ...
    <DropdownMenu ref="dropdown" slot="list">
      <template v-for="child in children">
        ...
        <DropdownItem v-else :key="`drop-${child.name}`" :name="child.perCode"><common-icon :size="iconSize" :type="child.clazz"/><span class="menu-title">{{ showTitle(child) }}</span></DropdownItem>
      </template>
    </DropdownMenu>
  </Dropdown>
</template>
